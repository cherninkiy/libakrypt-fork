#Libakrypt: перечень изменений


## Изменения в версии 0.10.1

 - минимальная версия cmake увеличена до 3.20
 - удалена возможность сборки библиотеки libakrypt-base и, как следствие, удален заголовочный файл libakrypt-base.h
 - опции библиотеки больше не сохраняются в файлы, а настраиваются на этапе компиляции библиотеки
 - добавлена возможность сборки библиотеки без стандартной библиотеки libc
   (примеры реализации такой возможности содержится в каталоге examples/nolibc-linux)
 - добавлена возможность фрагментарной сборки функций библиотеки
   (реализовано через опции cmake и настройку файла libakrypt-config.h)

 - изменена реализация функций аудита
 - функция ak_libakrypt_set_option() переименована в ak_libakrypt_set_option_by_name()
 - добавлена функция ak_libakrypt_set_option_by_index()
 - в ak_libakrypt_test_hash_functions() добавлено тестирование алгоритма crc64 


## Изменения в версии 0.9.18

 - исправлена старая ошибка при проверке входных параметров электронной подписи
 - исправлено обращение за пределы выделенной памяти в ходе реализации
   процесса чтения криптографических параметров библиотеки из конфигурационного файла
 - добавлена опция AK_LOAD_CONFIG для системы сборки cmake, которая позволяет отключить
   поиск и чтение значений криптографических параметров


## Изменения в версии 0.9.17

 - изменен механизм формирования заголовочных файлов библиотеки
   - добавлен libakrypt-config.h
   - зафиксирован libakrypt-base.h
 ...
 - исправлены ошибки, возникающие при компиляции библиотеки с флагами -Werror -Wextra -Wpedantic  
