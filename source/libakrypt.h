...
#define ak_error_external_function            (-37)
...
#ifdef AK_HAVE_OID
/*! @brief Инициализация контекста функции бесключевого хеширования по значению
 *  заданного идентификтора объекта.
    @param hctx Контекст функции хеширования.
    @param oid Контекст идентификатора алгоритма бесключевого хеширования.
    @return Функция возвращает код ошибки или @ref ak_error_ok (в случае успеха)                   */
 dll_export int ak_hash_create_oid( ak_hash hctx, ak_oid oid );
#endif
...
/*! @brief Внешняя функция выработки массива случайных данных заданного размера,
    ...
    качестве источника внешних случайных данных.                                                   */
 typedef int( ak_function_external_rng )( ak_pointer out_buf, size_t out_buf_size );
...
/*! @brief Инициализация контекста линейного конгруэнтного генератора псевдо-случайных чисел.
    ...
    возвращается код ошибки.                                                                       */
 dll_export int ak_random_create_lcg( ak_random generator );

/*! @brief Инициализация контекста нелинейного конгруэнтного генератора с обратной
    квадратичной связью (NLFSR генератора).                                                        */
 dll_export int ak_random_create_nlfsr( ak_random generator );

/*! @brief Инициализация контекста нелинейного конгруэнтного генератора с обратной квадратичной
    связью с явным указанием параметров генератора.                                                */
 dll_export int ak_random_create_nlfsr_with_params( ak_random generator,
                                                                size_t size, ak_uint64 linear_part );
#ifdef _WIN32
/*! @brief Инициализация контекста, реализующего интерфейс доступа к генератору
    псевдо-случайных чисел, предоставляемому ОС Windows.                                           */
 dll_export int ak_random_create_winrtl( ak_random generator );
#endif

/*! @brief Инициализация контекста генератора, реализованного
    на основе функции хеширования согласно Р 1323565.1.006-2017.                                   */
 dll_export int ak_random_create_hrng( ak_random generator );

/*! @brief Инициализация контекста генератора указателем на внешнюю по отношению к библиотеке
    функцию генерации случайных чисел.
    @param generator указатель на контекст генератора псевдо-случайных чисел
    @param function Указатель на внешнюю функцию-источник псевдо-случайных чисел.
    @return В случае успеха функция возвращает @ref ak_error_ok. В противном случае
    возвращается код ошибки.                                                                       */
 dll_export int ak_random_create_external_rng( ak_random generator,
                                                                ak_function_external_rng* function );
#ifdef AK_HAVE_OID
/*! @brief Инициализация контекста генератора по заданному OID
    алгоритма генерации псевдо-случайных чисел.                                                    */
 dll_export int ak_random_create_oid( ak_random, ak_oid );
#endif

/*! @brief Установка внутреннего состояния генератора псевдо-случайных чисел. Установка происходит
    посредством вызова функции-члена класса @ref random, которая и реализует механизм инициализации.
    ...
    @return В случае успеха функция возвращает @ref ak_error_ok. В противном случае
    возвращается код ошибки.                                                                       */
 dll_export int ak_random_randomize( ak_random generator, const ak_pointer in, const ssize_t size );

/*! @brief Выработка псевдо-случайных данных. Выработка
    происходит посредством вызова функции-члена класса @ref random.
    @param generator контекст генератора псевдо-случайных чисел.
    ...
    @return В случае успеха функция возвращает @ref ak_error_ok. В противном случае
    возвращается код ошибки.                                                                       */
 dll_export int ak_random_ptr( ak_random generator, const ak_pointer out, const ssize_t size );

/*! @brief Уничтожение данных, хранящихся в полях структуры @ref random.
    @param generator Указатель на структуру struct random
    @return В случае успеха возвращается ak_error_ok (ноль). В случае возникновения ошибки
    возвращается ее код.                                                                           */
 dll_export int ak_random_destroy( ak_random generator );
...
/*! @brief Некриптографическая функция генерации случайного 64-х битного целого числа.
    ...
    номер пользователя.
    @note Несмотря на кажущуюся случайность вырабатываемого значения, функция не должна
    использоваться для генерации значений, для которых требуется криптографическая случайность.
    Это связано с достаточно прогнозируемым изменением значений функции при многократных
    повторных вызовах.
    @note Основная задача данной функции - инициализация программного генератора
...
/*! @brief Функция очистки заданной области памяти памяти. */
 dll_export int ak_ptr_wipe( ak_pointer ptr, size_t size, ak_random rnd );

/*! @brief Статистическая проверка короткой двоичной последовательности (32, 64 или 128 байт) */
 dll_export bool_t ak_random_dynamic_test( ak_uint8 *data, size_t size );
