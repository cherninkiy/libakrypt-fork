...
@{ */
/*! \brief Идентификаторы множества алгоритмов выработки производного ключа */
 typedef enum {
  /*! \brief Неопределеный алгоритм, может служить признаком ошибки  */
   undefined_kdf = 0x00,

/*  Идентификаторы множества алгоритмов выработки производного ключа,
    определяемых рекомендациями Р 1323565.1.022-2018, раздел 5. */
...
/*  Идентификаторы множества алгоритмов выработки производного ключа,
    определяемых рекомендациями Р 50.1.113-2016, раздел 4.4. */
  /*! \brief Идентификатор алгоритма KDF256 */
   kdf256  = 0x91,
  /*! \brief Идентификатор алгоритма KDF512 */
   kdf512  = 0x92,
  /*! \brief Идентификатор алгоритма KDFNMAC */
   kdfnmac = 0xa1

} kdf_t;

/* ----------------------------------------------------------------------------------------------- */
/*! \brief Функция выработки производного ключа с использованием обобщенного алгоритма,
    регламентируемого в рекомендациях Р 50.1.113-2016, раздел 4.4. */
 dll_export int ak_skey_derive_kdf_hmac( kdf_t , ak_uint8 *, const size_t ,
                  ak_uint8 *, const size_t , ak_uint8 *, const size_t , ak_uint8 *, const size_t );
/*! \brief Функция выработки производного ключа с использованием обобщенного алгоритма,
    регламентируемого в рекомендациях Р 50.1.113-2016, раздел 4.4. */
 dll_export int ak_skey_derive_kdf_hmac_from_skey( kdf_t , ak_pointer ,
                  ak_uint8 *, const size_t , ak_uint8 *, const size_t , ak_uint8 *, const size_t );
/*! \brief Функция выработки производного ключа с использованием обобщенного алгоритма,
    регламентируемого в рекомендациях Р 50.1.113-2016, раздел 4.4. */
 dll_export int ak_skey_set_derive_kdf_hmac_from_skey( ak_pointer , kdf_t , ak_pointer ,
...
/*! \brief Контекст алгоритма генерации производной ключевой информации */
 typedef struct tlstree_state {
   ...
   /*! \brief внутренний механизм выработки производного ключа: kdf256, kdf512 или kdfnmac */
    kdf_t type;
 ...
 *  для заданного значения индекса. */
 dll_export int ak_tlstree_state_create( kdf_t , ak_tlstree_state ,
...
/*! \brief Функция TLSTREE для выработки производного ключа с использованием обобщенного алгоритма,
    регламентируемого в рекомендациях Р 1323565.1.030-2019, раздел 10.1.2.1 */
 dll_export int ak_skey_derive_tlstree( kdf_t , ak_uint8 *, const size_t ,
                                                ak_uint64 , tlstree_t , ak_uint8 *, const size_t );
/*! \brief Функция TLSTREE для выработки производного ключа с использованием обобщенного алгоритма,
    регламентируемого в рекомендациях Р 1323565.1.030-2019, раздел 10.1.2.1 */
 dll_export int ak_skey_derive_tlstree_from_skey( kdf_t , ak_pointer ,
                                                ak_uint64 , tlstree_t , ak_uint8 *, const size_t );
/*! \brief Функция TLSTREE для выработки производного ключа с использованием обобщенного алгоритма,
    регламентируемого в рекомендациях Р 1323565.1.030-2019, раздел 10.1.2.1 */
 dll_export int ak_skey_set_derive_tlstree_from_skey( ak_pointer , kdf_t ,
                                                              ak_pointer , ak_uint64 , tlstree_t );
/*! \brief Функция TLSTREE для выработки пары производных ключей с использованием обобщенного
    алгоритма, регламентируемого в рекомендациях Р 1323565.1.030-2019, раздел 10.1.2.1
    (явный вызов алгоритма TLSTREE512 )
 dll_export int ak_skey_set_pair_derive_tlstree_from_skey( ak_pointer , ak_pointer ,
                                                             ak_pointer , ak_uint64 , tlstree_t ); */
...
/*! \brief Функция выводит с помощью установленной функции аудита параметры эллиптической кривой */
 dll_export void ak_wcurve_print_to_log( ak_wcurve , int );
