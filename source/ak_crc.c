/* ----------------------------------------------------------------------------------------------- */
/*  Copyright (c) 2010, 2024 by Axel Kenzo, axelkenzo@mail.ru                                      */
/*                                                                                                 */
/*  Файл ak_hash.c                                                                                 */
...
/* ----------------------------------------------------------------------------------------------- */
/*                               Реализация функций класса crc64                                   */
/* ----------------------------------------------------------------------------------------------- */
 #define CRC64_ECMA182_POLY 0x42F0E1EBA9EA3693ULL

/* ----------------------------------------------------------------------------------------------- */
 static const ak_uint64 CRC64_TABLE[256] = {
...
/* ----------------------------------------------------------------------------------------------- */
 static int ak_hash_context_crc64_clean( ak_pointer data )
{
  if( data == NULL ) return ak_error_null_pointer;
  *(ak_uint64 *)data = 0;
...
/* ----------------------------------------------------------------------------------------------- */
 static int ak_hash_context_crc64_update( ak_pointer data, const ak_pointer in, const size_t size )
{
    size_t i = 0;
    const ak_uint8 *_p = in;
    ak_uint64 crc64 = *(ak_uint64 *)data;

    for( i = 0; i < size; i++ ) {
       ak_uint8 t = (( crc64 >> 56) ^ ( *_p++ )) & 0xFF;
       crc64 = CRC64_TABLE[t] ^ (crc64 << 8);
    }
    *(ak_uint64 *)data = crc64;

 return ak_error_ok;
}

/* ----------------------------------------------------------------------------------------------- */
 static int ak_hash_context_crc64_finalize( ak_pointer data,
                    const ak_pointer in, const size_t size, ak_pointer out, const size_t out_size )
{
   /* обрабатываем хвост */
    if(( in != NULL ) && ( size != 0 ))  ak_hash_context_crc64_update( data, in, size );
    memcpy( out, data, ak_min( 8, out_size ));
