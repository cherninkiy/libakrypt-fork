...
/** \defgroup buffer Функции для хранения буфферов или строк с данными
  @{ */
/*! \brief Структура для хранения буффера */
...
/*! \brief Функция присваивает буферу указатель на данные, к которым он имеет доступ, но не владеет */
 dll_export int ak_buffer_set_ptr( ak_buffer , ak_uint8 * , const size_t );
/*! \brief Функция помещает в буфер данные */
 dll_export int ak_buffer_alloc_ptr( ak_buffer , const ak_uint8 * , const size_t );
/*! \brief Функция освобождает данные */
...
/*! \brief Двусвязный список. */
 typedef struct list {
   ...
   /*! \brief количество содержащихся узлов в списке (одного уровня) */
    size_t count;
   /*! \brief функция удаления узла списка */
    ak_function_delete_object *free_data;
 } *ak_list;

/* ----------------------------------------------------------------------------------------------- */
/*! \brief Создание нового узла двусвязного списка, содержащего указатель произвольные данные. */
 dll_export ak_list_node ak_list_node_new( ak_pointer );
...
/*! \brief Удаление узла двусвязного списка. */
 dll_export ak_pointer ak_list_node_delete( ak_list, ak_list_node );
...
/*! \brief Установка пользовательской функции удаления данных, хранящихся в списке */
 dll_export int ak_list_set_delete_function( ak_list , ak_function_delete_object );
...
/* ----------------------------------------------------------------------------------------------- */
/** \defgroup htable Функции для работы с хэш-таблицами
  @{ */
/*! \brief Контекст одного элемента хэш-таблицы - пары (ключ:данные) */
 typedef struct keypair
{
 /*! \brief Указатель на общую область памяти для ключа и данных.
...
/*! \brief Функция, вычисляющая хэш-код от заданной области памяти */
 typedef size_t ( ak_function_get_hash_value )( ak_const_pointer , const size_t );
...
/* ----------------------------------------------------------------------------------------------- */
/*! \brief Функция создает ключевую пару - (ключ:данные) */
 dll_export int ak_keypair_create( ak_keypair ,
                               ak_const_pointer , const size_t , ak_const_pointer , const size_t );
/*! \brief Функция выделяет память и создает в ней ключевую пару - (ключ:данные) */
 dll_export ak_keypair ak_keypair_new( ak_const_pointer ,
                                                  const size_t , ak_const_pointer , const size_t );
/*! \brief Функция удаляет ключевую пару */
 dll_export int ak_keypair_destroy( ak_keypair );
/*! \brief Функция удаляет ключевую пару и освоюождает память */
 dll_export ak_pointer ak_keypair_delete( ak_pointer );

/* ----------------------------------------------------------------------------------------------- */
/*! \brief Функция создает хэш-таблицу с заданным числом узлов (списков) */
 dll_export int ak_htable_create( ak_htable , size_t );
/*! \brief Функция создает хэш-таблицу и инициализирует ее значения из заданного файла  */
 dll_export int ak_htable_create_from_file( ak_htable , const tchar * );
/*! \brief Функция выделяет память и создает в ней хэш-таблицу с заданным числом узлов (списков) */
 dll_export ak_htable ak_htable_new( size_t );
/*! \brief Установка пользовательской функции удаления данных, хранящихся в списке */
 dll_export int ak_htable_set_hash_function( ak_htable , ak_function_get_hash_value );
/*! \brief Функция добавляет в таблицу ключевую пару (ключ:данные) */
 dll_export int ak_htable_add_key_value( ak_htable ,
                               ak_const_pointer , const size_t , ak_const_pointer , const size_t );
/*! \brief Функция добавляет в таблицу ключевую пару (ключ:null-строка) */
 dll_export int ak_htable_add_key_str( ak_htable ,
                                                 ak_const_pointer , const size_t , const tchar * );
/*! \brief Функция добавляет в таблицу ключевую пару (null-строка:данные) */
 dll_export int ak_htable_add_str_value( ak_htable ,
                                                 const tchar * , ak_const_pointer , const size_t );
/*! \brief Функция добавляет в таблицу ключевую пару (null-строка:null-строка) */
 dll_export int ak_htable_add_str_str( ak_htable , const tchar * , const tchar * );
/*! \brief Функция возвращает указатель на ключевую пару по заданному ключу */
 dll_export ak_keypair ak_htable_get_keypair( ak_htable , ak_const_pointer , const size_t );
/*! \brief Функция возвращает указатель на ключевую пару по ключу, заданному null-строкой */
 dll_export ak_keypair ak_htable_get_keypair_str( ak_htable , const tchar * );
/*! \brief Функция возвращает указатель на данные по заданному ключу */
 dll_export ak_pointer ak_htable_get( ak_htable , ak_const_pointer , const size_t , size_t * );
/*! \brief Функция возвращает указатель на данные по ключу, заданному null-строкой */
 dll_export ak_pointer ak_htable_get_str( ak_htable , const tchar *, size_t * );


/*! \brief  Функция изымает текущюю ключевую пару из хэш-таблицы */
 dll_export ak_keypair ak_htable_exclude_keypair( ak_htable , ak_const_pointer , const size_t );
/*! \brief  Функция изымает текущюю ключевую пару из хэш-таблицы */
 dll_export ak_keypair ak_htable_exclude_keypair_str( ak_htable , const tchar * );

/*! \brief Функция возвращает количество элементов, помещенных в хэш-таблицу */
 dll_export size_t ak_htable_count( ak_htable );
/*! \brief Функция экспортирует хэш-таблицу в файл */
 dll_export int ak_htable_export_to_file( ak_htable , const tchar * );
/*! \brief Функция удаляет хэш-таблицу */
 dll_export int ak_htable_destroy( ak_htable );
/*! \brief Функция удаляет хэш-таблицу и освобождает выделенную память */
 dll_export ak_pointer ak_htable_delete( ak_pointer );
/** @} */
...
/*! \brief Определение функции, передаваемой в качестве аргумента в функцию построчного чтения файлов. */
 typedef int ( ak_function_file_read ) ( const char * , ak_pointer );
...
/*! \brief Функция построчного чтения заданного файла */
 dll_export int ak_file_read_by_lines( const tchar *, ak_function_file_read * , ak_pointer );
...
/*! \brief Вывод времени ( в секундах) в человекочитаемом формате. */
 dll_export char *ak_time_to_str( time_t );
