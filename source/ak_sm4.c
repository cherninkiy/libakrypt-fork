...
static int ak_sm4_delete_keys (ak_skey skey)
{
    ...
    if( skey->data != NULL ) {
        ak_ptr_wipe( skey->data, sizeof( struct sm4_encryped_keys), &skey->generator );
...
{
    struct sm4_encryped_keys *data = NULL;
    ...
    if(( data = ak_aligned_malloc( sizeof( struct sm4_encryped_keys ))) == NULL )
    ...
    memset( data, 0, sizeof( struct sm4_encryped_keys ));
    skey->data = ( ak_pointer )data;
    skey->flags |= key_flag_data_not_free;
    skey->generator.random( &skey->generator, data->sm4_expanded_key+32, 32*sizeof( ak_uint64 ));
    ak_uint32 MK[4];
    ak_uint32 mMK[4];
    ak_uint32 k[36];
...
static void ak_sm4_encrypt( ak_skey skey, ak_pointer in, ak_pointer out )
{
    ...
    while(i<32)
    {
        buf[i+4] = sm4_round_encrypt(buf[i], buf[i+1], buf[i+2], buf[i+3], ((struct sm4_encryped_keys*)(skey->data))->sm4_expanded_key[i]);
        i++;
    }
...
static void ak_sm4_decrypt( ak_skey skey, ak_pointer in, ak_pointer out )
{
    ...
    while(i<32)
    {
        buf[i+4] = sm4_round_encrypt(buf[i], buf[i+1], buf[i+2], buf[i+3], ((struct sm4_encryped_keys*)(skey->data))->sm4_expanded_key[31-i]);
        i++;
    }
    sm4_uint32_to_4uint8(buf[35],(ak_uint8*)out,0);
    sm4_uint32_to_4uint8(buf[34],(ak_uint8*)out,4);
