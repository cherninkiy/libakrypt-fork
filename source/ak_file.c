...
/*! \param filename Имя, для которого проводится проверка
    \return Функция возвращает одну из констант \ref DT_REG, \ref DT_DIR или \ref DT_CHR
    в случае успеха. В случае, если имя ошибочно, то возвращается \ref ak_error_access_file.       */
/* ----------------------------------------------------------------------------------------------- */
...
/* ----------------------------------------------------------------------------------------------- */
/*! Функция отражает в память, расположенную по адресу start, данные из файла file.
    Смещение от начала файла задется переменной offset, размер данных задается переменной length.
    Дескриптор отображаемого файла должен быть предварительно открыт на чтение или запись.

    Параметр prot описывает желаемый режим защиты памяти и не должен конфликтовать с режимом,
    в котором открыт файл. Значение prot должно быть
    равно PROT_NONE (доступ к этой области памяти запрещен), либо побитовым ИЛИ одного
    или нескольких флагов PROT_EXEC (данные в страницах могут исполняться),
    PROT_READ (данные можно читать), PROT_WRITE (в эту область можно записывать информацию).

    Параметр flags задает тип отражаемого объекта, опции отражения и указывает,
    принадлежат ли отраженные данные только этому процессу или их могут читать другие.
    Он состоит из комбинации (побитового ИЛИ) следующих битов
    MAP_SHARED (память разделяется с другими процессами),
    MAP_PRIVATE (неразделяемое отображение с механизмом copy-on-write).

   \note Фактически, функция просто предоставляет интерфейс к системному вызову mmap.

   \param file Контекст отображаемого файла
   \param start Начальный адрес отображения в память, по умолчанию - ноль
   \param length Количество отображаемых в память байт.
   \param prot Режим испольнения данных
   \param flags Режим доступа к ланным
   \param offset Смещение от начала файла
   \return Функция возвращает адрес (указатель на область памяти) в которую были размещены
    отображаемые данные.*/
/* ----------------------------------------------------------------------------------------------- */
