 ...
 int main( void )
{
 ...
 /* 3. вырабатываем случайный секретный ключ */
  ...
  if(( error = oid_of_target->func.first.set_key_random( key, generator )
	!= ak_error_ok )) {
    goto ex;
  }
 ...
 /* 6.  Заполняем информацию о владельце сертификата (в данном случае только CN) */
  ...
  if(( error = ak_tlv_add_string_to_global_name(
	subject, "common-name", "Root CA Name")) != ak_error_ok ) {
    goto ex;
  }
  cert.opts.subject = subject;

 /* 7. создаем сертификат  */
  cert.opts.ext_ca.is_present = cert.opts.ext_ca.value = ak_true;
  cert.opts.ext_key_usage.bits ^= bit_keyCertSign;
  cert.opts.ext_key_usage.is_present = ak_true;
  cert.opts.time = tm;
  if (( error = ak_certificate_export_to_file( &cert, key, &cert, generator,
                      "root_cert.crt", 0, asn1_pem_format )) != ak_error_ok ) {
	goto ex;
  }
 ...
 /* 8. сохраняем созданный ключ в файле */
  if(( error =  ak_skey_export_to_file_with_password(
          ...
          NULL,               /* пароль */
          0,                  /* длина пароля */
          "root_private.key", /* имя файла, куда будет записан секретный ключ */
          0,
          asn1_pem_format
     )) != ak_error_ok ) {
     goto ex;
