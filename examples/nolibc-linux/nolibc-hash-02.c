...
/*  внутреннего контекста функции хеширования Стрибог-256.                           */
/*                                                                                   */
 ...
 int main( void )
{
   ...
   /* инициализируем и обнуляем контекст */
     ctx.hsize = 32;
     ak_streebog_clean( &ctx );

   /* вычисляем хеш от заданного массива данных:
      в начале вычисляем данные, длина которых кратна длине блока */
     ak_streebog_update( &ctx, streebog_M2_message, 64 );
   /* в заключение, обрабатываем хвост */
     ak_streebog_finalize( &ctx, streebog_M2_message +64, 8, out, 32 );
