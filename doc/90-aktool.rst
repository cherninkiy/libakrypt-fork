...
t              test         Тестирование корректности и скорости работы
=============  ============ =========
...
предоставляющего в `Unix`-системах доступ к биологическому генератору случайных чисел, см. P 1323565.1.012-2017, раздел 3.4.
...
Для генерации ключей парной связи (общих симметричных ключей) в схеме Блома необходимо
...
Команда `icode` (короткая форма `i`) предназначена для контроля целостности
файлов, расположенных на файловых носителях, а также сегментов исполняемого кода, расположенных в памяти вычислительного средства.
Контроль целостности производится методом выработки и последующей
проверки кодов целостности или кодов аутентификации (имитовставок).

Команда `icode` может исполняться в двух режимах:

 * в режиме выработки кодов целостности или кодов аутентификации, помещаемых в заданную пользователем базу данных,
 * в режиме проверки кодов целостности или кодов аутентификации, выработанных ранее.

Для вычисления кодов целостности применяются алгоритмы бесключевого сжатия (функции хэширования).
Перечень доступных алгоритмов может быть получен при помощи вызова::

    aktool show --oid "hash function"

Для вычисления кодов аутентификации (имитовставок) применяются алгоритмы ключевого сжатия,
поэтому пользователем должен быть предварительно создан исходный секретный ключ. Данный ключ
вырабатывается с использованием команды `key`.

В настоящее время для вычисления имитовставок могут быть использованы следующие алгоритмы:

   * `cmac` (ГОСТ Р 34.13-2015),  `hmac` (Р 50.1.113-2016) и `nmac` (1323565.1.022-2018).

Поскольку некоторые алгоритмы вычисления кодов аутентификации (имитовставок) накладывают
ограничения на объем информации, обрабатываемой на одном ключе, то для каждого файла
в процессе работы команды `icode` вырабатывается свой уникальный производный ключ
(фомирование и последующее удалени производных ключей выполняется без участия пользователя).
Если появляется необходимость вычислять
коды аутентификации (имитовставки) для файлов с произвольной, сколь угодно большой длиной, то
необходимо использовать следующие алгоритмы: `hmac-streebog256`, `hmac-streebog512`.
...
.. option:: --add
Опция
добавлят к существующей базе данных с кодами целостности или кодами аутентификации

add new (коды аутентификации или целостности) to an existing database

.. option:: -a, --algorithm=алгоритм

Опция позволяет указать алгоритм бесключевого сжатия, используемого для вычисления
кодов целостности. В качестве параметра опции может выступать имя алгоритма, например `streebog512`, или его идентификатор.
...
Для алгоритмов ключевого сжатия данная опуия не применяется (используется опция `--key`).


--clean             cleaning the existing database with authentication or integrity codes
-c, --config            specify the name of the configuration file
-d, --database          specify the name of database with authentication or integrity codes
                    [ default name: /var/tmp/aktool.icodes]
--dont-show-icode   do not output calculated authentication or integrity codes to the console
--dont-show-stat    do not show a statistical results after creation or verification of integrity codes
-e, --exclude           specify the name of excluded files or directories
--exclude-link      specify a link to the file in the memory of the process that should be excluded
--format            set the format of output hash table [ enabled values: binary linux bsd, default: binary ]
--hash-table-nodes  number of high-level nodes in the generated hash table [ default: 1024 ]
...
-i, --input             set the name of file with previously created authentication or integrity codes
--key               specify the name of file with the secret key
                    this option also sets the type of keyed authentication mechanism
-l, --list              list the table of previously created authentication or integrity codes
--max-pid           set the maximal identifier of verified process [default: 2147483647]
--min-pid           set the minimal identifier of verified process [default: 1]
-n, --no-database       do not save the calculated authentication or integrity codes to the database
--no-derive         do not use the keyed authentication mechanism's derived key for each controlled entity
                    this may cause an error due to the exhaustion of a key resource
--offset            set the offset from the beginning of the file
                    defines the beginning of the file fragment being processed
--only-one-pid      verify only one process with given identifier (pid)
--only-segments     create or verify authentication or integrity codes only for downloadable segments
-p, --pattern           set the pattern which is used to find files
--pid               short form of --only-one-pid option
-r, --recursive         recursive search of files
--reverse-order     output of authentication or integrity code in reverse byte order
--search-deleted    additional search for deleted files in the process of verifying directories
--size              set the size of the file fragment being processed
                    the value -1 determines the size of the data to the end of the file
--tag               create a BSD-style hash table format
-v, --verify            verify previously created authentication or integrity codes
--with-segments     create or verify authentication or integrity codes for downloadable segments
...
и помещает результат вычислений в файл `result.txt`.::
  ...
  aktool i file.txt -d result.txt
