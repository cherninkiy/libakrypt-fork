...
Опция `AK_LOAD_OPTIONS` определяется в `CMakeLists.txt` следующим образом::

    option( AK_LOAD_OPTIONS "Loading library options from config file every time as library starts" ON )

Опция определяет откуда берутся значения криптографических параметров при старте библиотеки.
Значение `ON` заставляет библиотеку считывать данные значения из конфигурационного файла,
расположенного либо в домашнем каталоге пользователя, либо в каталоге,
устанавливаемым значением опции `AK_CONFIG_PATH`.
Значение `OFF` заставляет бибилиотеку использовать значения параметров, 
явно указанные в исходных текстах библиотеки (см. файл ``ak_options.c``).
...
LIBAKRYPT_HAVE_DEBUG_FUNCTIONS
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Опция не определяется в `CMakeLists.txt` и передается пользотвателем через командную строку
